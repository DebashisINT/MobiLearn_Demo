package com.breezemobilearndemo.features.reimbursement.presentation
...
import com.breezemobilearndemo.app.domain.ShopActivityEntity
import com.breezemobilearndemo.app.types.FragType
import com.breezemobilearndemo.app.utils.AppUtils
import com.breezemobilearndemo.app.utils.CustomTextWatcher
import com.breezemobilearndemo.app.utils.PermissionUtils
import com.breezemobilearndemo.app.utils.ProcessImageUtils_v1
...
import com.breezemobilearndemo.features.reimbursement.api.ReimbursementConfigRepoProvider
import com.breezemobilearndemo.features.reimbursement.api.applyapi.ApplyReimbursementRepoProvider
import com.breezemobilearndemo.features.reimbursement.api.configfetchapi.ReimbursementConfigFetchRepoProvider
import com.breezemobilearndemo.features.reimbursement.api.reimbursementshopapi.ReimbursementShopRepoProvider
import com.breezemobilearndemo.features.reimbursement.model.*
import com.breezemobilearndemo.features.reimbursement.model.reimbursement_shop.ReimbursementShopDataModel
import com.breezemobilearndemo.features.reimbursement.model.reimbursement_shop.ReimbursementShopResponseModel
...
// 4.0 Allow_past_days_for_apply_reimbursement functionality AppV 4.2.6 Puja    08/04/2024
class ReimbursementNFrag : BaseFragment(), DateAdapter.onPetSelectedListener, View.OnClickListener, TabLayout.OnTabSelectedListener, RadioGroup.OnCheckedChangeListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val view = inflater.inflate(R.layout.fragment_reimbursement_new_n, container, false)
...
//                    XLog.d("ReimbursementConfigApiResponse ERROR: " + error.localizedMessage)
    ...
    private fun attachbreezemobilearndemoData() {
...
//                    XLog.d("ReimbursementShopApiResponse ERROR: " + error.localizedMessage)
    ...
    private fun fetchConfigDetailsForAmt() {
        if (!AppUtils.isOnline(mContext)) {
            (mContext as DashboardActivity).showSnackMessage(getString(R.string.no_internet))
            return
        }
        ...
        inputModel.fuel_id = ""
        inputModel.travel_id = ""

        if(visitTypeId.equals("1"))
            inputModel.visittype_id = "3"
        else if(visitTypeId.equals("2"))
            inputModel.visittype_id = "4"
        else if(visitTypeId.equals("3"))
            inputModel.visittype_id = "2"
                ...
                .subscribe({ result ->
                    ...
                    if (configResponse.status == NetworkConstant.SUCCESS) {
                        ...
                        if (!TextUtils.isEmpty(configResponse.maximum_allowance)) {
                            ...
                            //maximum_amount_allowance_Per_Km_TV.text = "\u20B9 ${configResponse.maximum_allowance}"
                            allowance_amount_LL.visibility = View.GONE
                            amount_EDT.setText("${configResponse.maximum_allowance}")
                        } else {
                            allowance_amount_LL.visibility = View.GONE
                        }
    ...
    */
            R.id.mode_of_travel_RL -> {
                if (conveyancePopupWindow != null && conveyancePopupWindow!!.isShowing)
                    ...
                    callExpenseTypeDropDownPopUp(mode_of_travel_RL, modeOfTravelArrayList, mode_of_travel_type_TV, resources.getDimensionPixelOffset(
                        R.dimen._270sdp))
            }
            ...
            R.id.fuel_type_value_TV -> {
                if (conveyancePopupWindow != null && conveyancePopupWindow!!.isShowing)
                    ...
                    callExpenseTypeDropDownPopUp(fuel_type_RL, fuelTypesArrayList, fuel_type_value_TV, resources.getDimensionPixelOffset(
                        R.dimen._270sdp))
            }
...
//end mantis id 26741 Storage permission updation Suman 22-08-2023
    ...
    private fun defaultSelectionOfTravelledModeN() {
    ...
    private fun getTime(textView: AppCustomTextView) {
        ...
        TimePickerDialog(mContext, R.style.DatePickerTheme, timeSetListener, cal.get(Calendar.HOUR_OF_DAY), cal.get(
    ...
    private fun callLocationDropDownPopUp(anchorView: View, arr_themes: ArrayList<String>, textView: AppCustomTextView, width: Int) {
        ...
        if (conveyancePopupWindow != null && !conveyancePopupWindow?.isShowing!!) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                conveyancePopupWindow?.showAtLocation(anchorView, Gravity.TOP, 0, resources.getDimensionPixelOffset(
                    R.dimen._10sdp))
            } else {
        ...
        if (conveyancePopupWindow != null && !conveyancePopupWindow?.isShowing!!) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ...
                rl_add_reimbursement_main.post(Runnable {
                    conveyancePopupWindow?.showAsDropDown(anchorView, resources.getDimensionPixelOffset(
                        R.dimen._10sdp), 0, Gravity.BOTTOM)
                })
            } else {
                conveyancePopupWindow?.showAsDropDown(anchorView, anchorView.width - conveyancePopupWindow?.width!!, 0)
            }
...
//                    XLog.d("Fetch ReimbursementConfigApiResponse : " + "\n" + "Status===> " + configResponse.status + ", Message===> " + configResponse.message)
                    ...
                    if (configResponse.status == NetworkConstant.SUCCESS) {
                        ...
                        if (!TextUtils.isEmpty(configResponse.rate)){
                            rate = configResponse.rate!!
                            amount_EDT.setText(rate.toString())
                        }
...
//                    XLog.d("Fetch ReimbursementConfigApiResponse ERROR: " + error.localizedMessage)
    ...
    private fun applyReimbursementApi() {
        ...
        if(visitTypeId.equals("1"))
             ...
             reinbursementInput.visit_type_id = "4"
        else if(visitTypeId.equals("3"))
             reinbursementInput.visit_type_id = "2"
        ...
        if (hotel_root_LL.visibility == View.VISIBLE) {
            ...
            reimbursementDetails.start_date_time =
                AppUtils.convertFromRightToReverseFormatWithTime(checked_in_date_TV.text.toString() + " " + checked_in_time_TV.text.toString())
            reimbursementDetails.end_date_time =
                AppUtils.convertFromRightToReverseFormatWithTime(checked_out_date_TV.text.toString() + " " + checked_out_time_TV.text.toString())
        } else {
...
//                    XLog.d("Delete Reimbursement Api Response : " + "\n" + "Status====> " + configResponse.status + ", Message===> " + configResponse.message)
                    ...
                    if (configResponse.status == NetworkConstant.SUCCESS) {
                        ...
                        if (reimbursmentTaList.size == 0) {
                            ...
                            layoutParams.setMargins(0, 0, 0, mContext.resources.getDimensionPixelOffset(
                                R.dimen._5sdp));
...
//                    XLog.d("Delete Reimbursement Api ERROR: " + error.localizedMessage)
    ...
    private fun showDeleteAlert(adapterPosition: Int) {
        ...
        CommonDialog.getInstance("Delete Alert", "Do you really want to delete this TA?", getString(
            R.string.cancel), getString(R.string.ok), object : CommonDialogClickListener {
