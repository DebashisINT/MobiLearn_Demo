package com.breezemobilearndemo.features.report.presentation
...
import com.breezemobilearndemo.features.report.api.report_api.ReportRepoProvider
import com.breezemobilearndemo.features.report.model.AchievementDataModel
import com.breezemobilearndemo.features.report.model.AchievementResponseModel
 ...
 */
class AchievementReportFragment : BaseFragment(), View.OnClickListener, DatePickerDialog.OnDateSetListener {
    ...
    private fun getVisitReport(fromDate: String, toDate: String) {
        ...
        BaseActivity.compositeDisposable.add(
                repository.getAchvReport(fromDate, toDate)
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribeOn(Schedulers.io())
                        .subscribe({ result ->
                            val response = result as AchievementResponseModel
                            when {
                                response.status == NetworkConstant.SUCCESS -> {
                                    progress_wheel.stopSpinning()
                                    initAdapter(response.achv_report_list)
    ...
    private fun initAdapter(achv_report_list: ArrayList<AchievementDataModel>?) {

        tv_no_data_available.visibility = View.GONE

        rv_achv_list.adapter = AchievementAdapter(mContext, achv_report_list, object : AchievementAdapter.OnClickListener {
            override fun onViewClick(adapterPosition: Int) {

                if (achv_report_list?.get(adapterPosition)?.achv_details_list?.size == 0)
                    (mContext as DashboardActivity).showSnackMessage(getString(R.string.no_data_available))
                else
                    (mContext as DashboardActivity).loadFragment(FragType.AchievementDetailsFragment, true, achv_report_list?.get(adapterPosition)!!)
            }
        })
    }
