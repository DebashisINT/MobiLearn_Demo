...
import com.breezemobilearndemo.app.domain.AreaListEntity
 ...
 */
class AreaListAdapter(private val context: Context, areaList: ArrayList<AreaListEntity>?, private val onItemClick: (AreaListEntity) -> Unit?,
                      private val onShowList: (Boolean) -> Unit) : RecyclerView.Adapter<AreaListAdapter.ViewHolder>(), Filterable {

    private val inflater: LayoutInflater
    private var mAreaList: ArrayList<AreaListEntity>? = null
    private var tempAreaList: ArrayList<AreaListEntity>? = null
    private var filterAreaList: ArrayList<AreaListEntity>? = null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.list_item_tv.text = mAreaList?.get(position)?.area_name
    ...
    inner class SearchFilter : Filter() {
        override fun performFiltering(p0: CharSequence?): FilterResults {
            ...
            tempAreaList?.indices!!
                    .filter { tempAreaList?.get(it)?.area_name?.toLowerCase()?.contains(p0?.toString()?.toLowerCase()!!)!! }
        ...
        override fun publishResults(p0: CharSequence?, results: FilterResults?) {
            ...
            try {
                filterAreaList = results?.values as ArrayList<AreaListEntity>?
                mAreaList?.clear()
                val hashSet = HashSet<String>()
                if (filterAreaList != null) {

                    filterAreaList?.indices!!
                            .filter { hashSet.add(filterAreaList?.get(it)?.area_id!!) }
