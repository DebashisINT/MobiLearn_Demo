...
class EntityTypeAdapter(private val context: Context, list: ArrayList<EntityTypeEntity>?,
                        private val onItemClick: (EntityTypeEntity) -> Unit?) : RecyclerView.Adapter<EntityTypeAdapter.ViewHolder>(), Filterable {

    private val inflater: LayoutInflater
    private var mList: ArrayList<EntityTypeEntity>? = null
    private var tempList: ArrayList<EntityTypeEntity>? = null
    private var filterList: ArrayList<EntityTypeEntity>? = null
    ...
    inner class SearchFilter : Filter() {
        ...
        override fun publishResults(p0: CharSequence?, results: FilterResults?) {
            ...
            try {
                filterList = results?.values as ArrayList<EntityTypeEntity>?
                mList?.clear()
                val hashSet = HashSet<String>()
                if (filterList != null) {

                    filterList?.indices!!
                            .filter { hashSet.add(filterList?.get(it)?.entity_id!!) }
