...
class DealerAdapter(private val context: Context, list: ArrayList<DealerEntity>?,
                    private val onItemClick: (DealerEntity) -> Unit?) : RecyclerView.Adapter<DealerAdapter.ViewHolder>(), Filterable {

    private val inflater: LayoutInflater
    private var mList: ArrayList<DealerEntity>? = null
    private var tempList: ArrayList<DealerEntity>? = null
    private var filterList: ArrayList<DealerEntity>? = null
    ...
    inner class SearchFilter : Filter() {
        ...
        override fun publishResults(p0: CharSequence?, results: FilterResults?) {
            ...
            try {
                filterList = results?.values as ArrayList<DealerEntity>?
                mList?.clear()
                val hashSet = HashSet<String>()
                if (filterList != null) {

                    filterList?.indices!!
                            .filter { hashSet.add(filterList?.get(it)?.dealer_id!!) }
