package com.breezemobilearndemo.features.myjobs.presentation
...
import com.breezemobilearndemo.features.myjobs.api.MyJobRepoProvider
import com.breezemobilearndemo.features.myjobs.model.*
...
class WorkOnHoldFragment : BaseFragment(), View.OnClickListener {
    ...
    companion object {
        fun newInstance(mcustomerdata: Any): WorkOnHoldFragment {
            val fragment = WorkOnHoldFragment()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val view = inflater.inflate(R.layout.fragment_work_hold, container, false)
    ...
    private fun initView(view: View) {
        view.apply {
            ...
            et_hold_reason = findViewById(R.id.et_hold_reason)
            et_remarks = findViewById(R.id.et_remarks)
            submit_button_TV = findViewById(R.id.submit_button_TV)
            progress_wheel = findViewById(R.id.progress_wheel)
            rl_work_hold_main = findViewById(R.id.rl_work_hold_main)
            tv_service_completed_header = findViewById(R.id.tv_service_completed_header)
        }

        if (customerdata?.statusCode == "2")
            tv_service_completed_header.text = getString(R.string.enter_reason_unhold)
    ...
    private fun initClickListener() {
        ...
        rl_work_hold_main.setOnClickListener(null)
    }
...
//end mantis id 26741 Storage permission updation Suman 22-08-2023
    ...
    private fun checkValidation() {
        when {
            ...
            TextUtils.isEmpty(et_hold_reason.text.toString().trim()) -> {
                if (customerdata?.statusCode == "2")
                    (mContext as DashboardActivity).showSnackMessage(getString(R.string.error_enter_reason_unhold))
                else
                    (mContext as DashboardActivity).showSnackMessage(getString(R.string.error_enter_reason_hold))
            }
            else -> {
                if (customerdata?.statusCode == "2")
                    submitWorkUnholdApi()
    ...
    private fun submitWorkUnholdApi() {
        if (!AppUtils.isOnline(mContext)) {
            (mContext as DashboardActivity).showSnackMessage(getString(R.string.no_internet))
            return
        }

        val wohInput = WorkUnHoldInputParams(Pref.session_token!!, Pref.user_id!!, customerdata?.id!!, selectedDate, tv_time.text.toString().trim(),
                et_hold_reason.text.toString().trim(), et_remarks.text.toString().trim(), AppUtils.getCurrentISODateTime(),
                Pref.current_latitude, Pref.current_longitude, LocationWizard.getNewLocationName(mContext, Pref.current_latitude.toDouble(), Pref.current_longitude.toDouble()))
        ...
        if (!TextUtils.isEmpty(et_attachment.text.toString().trim()) || !TextUtils.isEmpty(et_photo.text.toString().trim())) {
            ...
            BaseActivity.compositeDisposable.add(
                    repository.submitWorkUnhold(wohInput, imgList, mContext)
            ...
            BaseActivity.compositeDisposable.add(
                    repository.submitWorkUnhold(wohInput)
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribeOn(Schedulers.io())
                            .subscribe({ result ->
                                val response = result as BaseResponse
    ...
    private fun submitWOHApi() {
        if (!AppUtils.isOnline(mContext)) {
            (mContext as DashboardActivity).showSnackMessage(getString(R.string.no_internet))
            return
        }

        val wohInput = WorkOnHoldInputParams(Pref.session_token!!, Pref.user_id!!, customerdata?.id!!, selectedDate, tv_time.text.toString().trim(),
                et_hold_reason.text.toString().trim(), et_remarks.text.toString().trim(), AppUtils.getCurrentISODateTime(),
                Pref.current_latitude, Pref.current_longitude, LocationWizard.getNewLocationName(mContext, Pref.current_latitude.toDouble(), Pref.current_longitude.toDouble()))
        ...
        if (!TextUtils.isEmpty(et_attachment.text.toString().trim()) || !TextUtils.isEmpty(et_photo.text.toString().trim())) {
            ...
            val repository = MyJobRepoProvider.jobMultipartRepoProvider()
            BaseActivity.compositeDisposable.add(
                    repository.submitWOH(wohInput, imgList, mContext)
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribeOn(Schedulers.io())
                            .subscribe({ result ->
                                val response = result as BaseResponse
