...
class TypeListAdapter(private val context: Context, list: ArrayList<TypeEntity>?,
                      private val onItemClick: (TypeEntity) -> Unit?) : RecyclerView.Adapter<TypeListAdapter.ViewHolder>(), Filterable {

    private val inflater: LayoutInflater
    private var mList: ArrayList<TypeEntity>? = null
    private var tempList: ArrayList<TypeEntity>? = null
    private var filterList: ArrayList<TypeEntity>? = null
    ...
    inner class SearchFilter : Filter() {
        ...
        override fun publishResults(p0: CharSequence?, results: FilterResults?) {
            ...
            try {
                filterList = results?.values as ArrayList<TypeEntity>?
                mList?.clear()
                val hashSet = HashSet<String>()
                if (filterList != null) {

                    filterList?.indices!!
                            .filter { hashSet.add(filterList?.get(it)?.type_id!!) }
