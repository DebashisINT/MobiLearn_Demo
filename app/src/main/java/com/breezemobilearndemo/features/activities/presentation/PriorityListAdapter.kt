...
class PriorityListAdapter(private val context: Context, list: ArrayList<PriorityListEntity>?,
                          private val onItemClick: (PriorityListEntity) -> Unit?) : RecyclerView.Adapter<PriorityListAdapter.ViewHolder>(), Filterable {

    private val inflater: LayoutInflater
    private var mList: ArrayList<PriorityListEntity>? = null
    private var tempList: ArrayList<PriorityListEntity>? = null
    private var filterList: ArrayList<PriorityListEntity>? = null
    ...
    inner class SearchFilter : Filter() {
        ...
        override fun publishResults(p0: CharSequence?, results: FilterResults?) {
            ...
            try {
                filterList = results?.values as ArrayList<PriorityListEntity>?
                mList?.clear()
                val hashSet = HashSet<String>()
                if (filterList != null) {

                    filterList?.indices!!
                            .filter { hashSet.add(filterList?.get(it)?.priority_id!!) }
