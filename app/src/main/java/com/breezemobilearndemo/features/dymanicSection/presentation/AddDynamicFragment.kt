package com.breezemobilearndemo.features.dymanicSection.presentation
...
import com.breezemobilearndemo.features.dymanicSection.api.DynamicRepoProvider
import com.breezemobilearndemo.features.dymanicSection.model.*
...
class AddDynamicFragment : BaseFragment() {
    ...
    companion object {
        fun newInstance(id: Any): AddDynamicFragment {
...
//end mantis id 26741 Storage permission updation Suman 22-08-2023
    ...
    private fun saveData() {
        ...
        val dynamicInput = DynamicSubmitInputParams(Pref.session_token!!, Pref.user_id!!, id, "", dynamicList)

        if (TextUtils.isEmpty(dataPath)) {
            val repository = DynamicRepoProvider.dynamicRepoProvider()
            progress_wheel.spin()
            BaseActivity.compositeDisposable.add(
                    repository.saveDynamicData(dynamicInput)
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribeOn(Schedulers.io())
                            .subscribe({ result ->
...
//                                XLog.d("ADD DYNAMIC ERROR=======> " + error.localizedMessage)
            ...
            progress_wheel.spin()
            BaseActivity.compositeDisposable.add(
                    repository.addDynamicDataMultipart(dynamicInput, dataPath, mContext)
                            .observeOn(AndroidSchedulers.mainThread())
                            .subscribeOn(Schedulers.io())
                            .subscribe({ result ->
