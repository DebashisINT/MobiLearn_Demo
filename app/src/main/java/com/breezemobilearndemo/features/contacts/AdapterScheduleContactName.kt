...
class AdapterScheduleContactName(var mContext:Context, var customerList:ArrayList<ScheduleContactDtls>, var listner:onClick, private val getSize: (Int) -> Unit):
    ...
    private var mList: ArrayList<ScheduleContactDtls>? = null
    private var tempList: ArrayList<ScheduleContactDtls>? = null
    private var filterList: ArrayList<ScheduleContactDtls>? = null
    ...
    inner class SearchFilter : Filter() {
        ...
        override fun publishResults(p0: CharSequence?, results: FilterResults?) {
            ...
            try {
                filterList = results?.values as ArrayList<ScheduleContactDtls>?
                mList?.clear()
                val hashSet = HashSet<String>()
                if (filterList != null) {

                    filterList?.indices!!
                        .filter { hashSet.add(filterList?.get(it)?.contact_name!!) }
    ...
    fun refreshList(list: ArrayList<ScheduleContactDtls>) {
        mList?.clear()
        mList?.addAll(list)
    ...
    interface onClick{
        fun onTickUntick(obj:ScheduleContactDtls,isTick: Boolean)
    }
