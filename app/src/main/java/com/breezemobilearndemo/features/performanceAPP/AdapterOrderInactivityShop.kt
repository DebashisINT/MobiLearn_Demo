...
import com.breezemobilearndemo.features.performanceAPP.NoVisitedActivityInshop
...
class AdapterOrderInactivityShop(var mContext: Context, var mList:ArrayList<NoOrderTakenShop2>, var isFromCollection:Boolean=false, var isFromVisit:Boolean=false):
    RecyclerView.Adapter<AdapterOrderInactivityShop.CrossSellProductViewHolder>(){
    ...
    inner class CrossSellProductViewHolder(itemview: View): RecyclerView.ViewHolder(itemview){
        ...
        fun bindItems(){
            ...
            try{
                ...
                try{
                    itemView.tv_row_shop_list_tv_text_dynamic.text = "last order date"
                    itemView.tv_row_shop_list_ma_shop_count.text =  AppUtils.getFormatedDateNew( mList.get(adapterPosition).orderDate, "yyyy-mm-dd", "dd-mm-yyyy")
                }catch (ex:Exception){
                    itemView.tv_row_shop_list_tv_text_dynamic.text = "last order date"
