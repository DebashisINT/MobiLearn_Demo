...
import com.breezemobilearndemo.app.domain.ClientListEntity
import com.breezemobilearndemo.features.timesheet.model.TimeSheetClientDataModel
 ...
 */
class ClientListDialog : DialogFragment() {
    ...
    companion object {
        ...
        private var clientList: ArrayList<ClientListEntity>? = null
        private lateinit var onClick: (ClientListEntity) -> Unit

        fun newInstance(list: ArrayList<ClientListEntity>?, mOnClick: (ClientListEntity) -> Unit): ClientListDialog {
            val dialogFragment = ClientListDialog()
    ...
    private fun initView(v: View) {
        ...
        adapter = ClientAdapter(mContext, clientList!!, { client: ClientListEntity ->
            onClick(client)
            dismiss()
        })

        rv_common_dialog_list.adapter = adapter
        dialog_header_TV.text = "Client List"
