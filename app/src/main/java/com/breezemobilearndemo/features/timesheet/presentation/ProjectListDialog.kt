...
import com.breezemobilearndemo.app.domain.ProjectListEntity
import com.breezemobilearndemo.features.timesheet.model.TimeSheetProjectDataModel
 ...
 */
class ProjectListDialog: DialogFragment() {
    ...
    companion object {
        ...
        private lateinit var onClick: (ProjectListEntity) -> Unit
        private var projectList: ArrayList<ProjectListEntity>? = null

        fun newInstance(list: ArrayList<ProjectListEntity>?, mOnClick: (ProjectListEntity) -> Unit): ProjectListDialog {
            val dialogFragment = ProjectListDialog()
    ...
    private fun initView(v: View) {
        ...
        adapter = ProjectAdapter(mContext, projectList!!, { project: ProjectListEntity ->
            onClick(project)
            dismiss()
        })

        rv_common_dialog_list.adapter = adapter
        dialog_header_TV.text = "Project List"
