...
class AutoStartHelper {
    ...
    private fun autoStartAsus(context: Context) {
        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {
            showAlert(context, DialogInterface.OnClickListener { dialog: DialogInterface, which: Int ->
                try {
    ...
    private fun autoStartOppo(context: Context) {
        if (isPackageExists(context, PACKAGE_OPPO_MAIN) || isPackageExists(context, PACKAGE_OPPO_FALLBACK)) {
            showAlert(context, DialogInterface.OnClickListener { dialog, which ->
                try { //PrefUtil.writeBoolean(context, PrefUtil.PREF_KEY_APP_AUTO_START, true);
